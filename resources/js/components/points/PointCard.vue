...
<template>
	<div class="large-3 medium-4 small-6 cell point-card-container" v-show="show">
		<router-link :to="{ name: 'point', params: { id: point.id} }" v-on:click.native="panToLocation(point)">
			<div class="point-card">
				<span class="title">{{ point.name }}</span>
				<span class="address">
					<span class="street">{{ point.address }}</span>
				</span>
			</div>
		</router-link>
	</div>
...
<script>
	...
	import { PointCategoriesFilter } from '../../mixins/filters/PointCategoriesFilter.js';

	export default {
		/*
			The component accepts one point as a property
		*/
		props: ['point'],
		/*
		...
		methods: {
			...
			*/
			processFilters: function() {
				if (this.categoriesFilter.length == 0 || this.processPointCategoriesFilter(this.point, this.categoriesFilter)) {
					this.show = true;
				} else {
					this.show = false;
				}
